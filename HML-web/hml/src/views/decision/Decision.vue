  ...
  methods: {
    ...
    convertInteractionsToTree (interactions) {
      ...
      interactions.forEach(interaction => {
        ...
        // for (let i = 0; i < this.totalActions; i++) { // 假设totalActions是所有可能actions的数量
        for (let i = 0; i < 106; i++) { // 假设totalActions是所有可能actions的数量
    ...
    renderDecisionTree (treeData) {
      ...
      const treeLayout = d3.tree().size([900, 900]) // 调整为较小的尺寸
      ...
      const nodes = svg.selectAll('.node')
        ...
        .enter()
        .append('g')
        .attr('transform', d => `translate(${d.x}, ${d.y})`) // 竖向布局
      nodes.append('circle')
        .attr('class', 'node')
        .attr('r', 5) // 缩小节点大小
    ...
    // // 1023：决策树
    // fetchDecisionTree11 () {
    //   decisionApi.fetchDecisionTree1().then(response => {
    //     const resp = response.data
    //     if (resp.meta.code === 204) {
    ...
    // 1023：决策树
    showTree1Dialog () {
      this.fetchDecisionTree1()
      this.tree1DialogVisible = true
    },
    // 1023：决策树
    showTree2Dialog () {
      this.fetchDecisionTree2()
      this.tree2DialogVisible = true
    },
    // 1023：决策树
