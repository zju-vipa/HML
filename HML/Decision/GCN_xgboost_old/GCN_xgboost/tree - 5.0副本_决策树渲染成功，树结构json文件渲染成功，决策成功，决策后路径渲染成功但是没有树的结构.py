...
#         node_id = str(node_id)  # 确保 node_id 是字符串
#         node_info = tree_json.get(str(node_id), {})
#         y = -depth  # 因为我们希望树向下生长，所以使用负的深度值
...
#         label = info['condition']
#         color = 'green' if info['is_in_path'] == 'Yes' else 'red'
#         G.add_node(str(node_id), label=label, color=color)  # 将 node_id 转换为字符串
#
#     for node_id, info in tree_json.items():
#         for edge_label, child_id in [('yes', info['yes']), ('no', info['no'])]:
#             if child_id in tree_json:
#                 G.add_edge(str(node_id), str(child_id), label=edge_label)  # 将 node_id 和 child_id 转换为字符串
#
#     arrange_tree("0")  # 假设根节点的ID是0
#     G.layout(prog='neato', args='-n')  # 使用'neato'布局引擎并保持手动布局的节点位置
#     G.draw(save_path)
