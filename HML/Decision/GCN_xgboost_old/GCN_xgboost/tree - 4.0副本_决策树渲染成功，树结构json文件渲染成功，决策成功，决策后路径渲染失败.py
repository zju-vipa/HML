...
def plot_annotated_decision_tree(tree_json, save_path):
    ...
    for node_id, info in tree_json.items():
        G.add_node(node_id, label=info['condition'], is_in_path=info['is_in_path'])
        for edge_label, child_id in [('yes', info['yes']), ('no', info['no']), ('missing', info['missing'])]:
    ...
    nx.draw(G, pos, with_labels=True, labels=node_labels, node_color=['green' if info['is_in_path'] == 'Yes' else 'red' for node_id, info in G.nodes(data=True)])
