...
class TransmissionSectionEnv(gym.Env):
    ...
    def reset(self):
        if self.env_id == 'M5case9241' and self.method == 'MAM':
            ...
            if self.evaluation:
                ...
                self.current_idx = np.random.choice(self.ava_net_idx)  # 分断面样本

        self.current_net = self._load_net(self.current_idx)  # 初始化电网PQ值
        self.current_net_undo = copy.deepcopy(self.current_net)  # 保存电网副本
        self.current_power_section = copy.deepcopy(self.control_nets['control_nets_power_section'][self.current_idx])  # 断面潮流大小
    ...
    def step(self, action):
        ...
        self.current_power_section = self._get_power_section()  # 断面潮流大小
