    ...
    def recurse(node, depth):
        ...
        if tree_.feature[node] != _tree.TREE_UNDEFINED:
            ...
            return {
                ...
                'missing': recurse(left_child, depth + 1)
            }
        else:
...
def flatten_tree(decision_tree, feature_names=None):
    ...
    def _recurse(node_id):
        ...
        if tree.feature[node_id] != _tree.TREE_UNDEFINED:
            ...
            node['yes'] = int(tree.children_left[node_id])
            node['no'] = int(tree.children_right[node_id])
            node['missing'] = int(tree.children_left[node_id])
            flat_tree[node_id_str] = node
            _recurse(tree.children_left[node_id])
            _recurse(tree.children_right[node_id])
        else:
            node['value'] = tree.value[node_id].tolist()
            flat_tree[node_id_str] = node
...
def main():
    ...
    plt.savefig('decision_tree.png', format='png', dpi=1000)
    # plt.show()
