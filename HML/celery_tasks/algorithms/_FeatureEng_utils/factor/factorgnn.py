...
class factorgnn(nn.Module):
    ...
    def load_model(self, model_path):
        ...
        graph_structure = pickle.load(open(os.path.join(model_path, 'graph_structure.pkl'), 'rb'))
        self.out_feats = 1
        model = FactorGNN(graph_structure, in_feats, self.latent_dims, 1, 4, 0)
        model.load_state_dict(torch.load(os.path.join(model_path, 'factor.pth')))
        return model

    def test(self, model, data_path):
        model.eval()
        file_list = []
