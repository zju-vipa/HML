    ...
    def train(self) -> None:
        ...
        if self.use_ask_loss:
            def get_ask_observations(rollout_data):
                with th.no_grad():
                    _, latent_vf, _ = self.policy._get_latent(rollout_data.observations)
            ...
            for epoch in range(self.n_epochs):
                for rollout_data in self.rollout_buffer.get(self.batch_size):
                    ...
                    if ask_observations_num > 0:
                        ...
                        latent_pi, _, latent_sde = self.policy._get_latent(ask_observations)
                        distribution = self.policy._get_action_dist_from_latent(latent_pi, latent_sde)
                        prob = distribution.distribution.probs
                        ask_actions = th.as_tensor([self.env.action_space.n - 1] * ask_observations_num).long().to(self.device)
                        ask_loss = F.nll_loss(prob, ask_actions)
        ...
        for epoch in range(self.n_epochs):
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                if human_actions_num > 0:
                    ...
                    distribution = self.policy._get_action_dist_from_latent(latent_pi, latent_sde)
                    prob = distribution.distribution.probs
                    human_loss = F.nll_loss(prob[human_actions != -1], human_actions[human_actions != -1])
